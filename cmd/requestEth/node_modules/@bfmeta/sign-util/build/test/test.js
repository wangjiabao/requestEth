...
(async () => {
    const cryptoHelper = new cryptoHelper_1.CryptoHelper();
    const bfmetaSignUtil = new src_1.BFMetaSignUtil("c", Buffer, cryptoHelper);
    // 生成公私钥对
    ...
    // 获取安全公钥
    const secondPublicKey = await bfmetaSignUtil.getSecondPublicKeyStringFromSecretAndSecondSecret("123", "1");
    console.log("获取安全公钥", secondPublicKey);
    // 获取安全公钥
    const secondPublicKeyV2 = await bfmetaSignUtil.getSecondPublicKeyStringFromSecretAndSecondSecretV2("123", "1");
    ...
    const address = await bfmetaSignUtil.getAddressFromPublicKeyString(keypair.publicKey.toString("hex"));
