...
const index_js_2 = require("../extends-promise-safe/index.js");
/**
 * 并发池
 */
class ParallelPool {
    constructor(maxParallelNum = 2) {
        Object.defineProperty(this, "maxParallelNum", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: maxParallelNum
        });
        /**正在执行中的任务集合 */
        Object.defineProperty(this, "_runningTasks", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        /**等待执行队列 */
        Object.defineProperty(this, "_waitingQuene", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "_yielding", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    addTaskExecutor(executor) {
        const po = new index_js_1.PromiseOut();
     ...
     */
    get isFull() {
        return (this._runningTasks.length + this._waitingQuene.length >=
            this.maxParallelNum);
    }
    /**执行一条并发线 */
    ...
    async *_yieldResults(opts = {}) {
        /**空闲并发线 */
        const freeParallelList = [];
        ...
        do {
             ...
             */
            while (parallelTaskList.length < this.maxParallelNum &&
            ...
            /// 等任务被填满，就可以进行返回
            const result = await (0, index_js_2.safePromiseRace)(parallelTaskList);
            if (result) {
                yield result.asset;
                result.rm();
            }
