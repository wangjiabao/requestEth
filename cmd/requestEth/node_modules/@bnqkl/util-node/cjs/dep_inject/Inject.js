...
exports.InjectProp = exports.DirtyInjectProp = exports.Inject = void 0;
 ...
 * @param module_name
 */
function Inject(inject_module, conf) {
    return (target, prop_name, index) => {
        if (typeof index === "number") {
            const module_name = typeof inject_module === "function"
                ? (0, common_js_1.getInjectionToken)(inject_module)
                : inject_module;
            if (module_name !== undefined) {
                const modules = _inner_js_1.FACTORY_CUSTOM_INJECT_MODULE_MAP.get(target) || new Map();
                _inner_js_1.FACTORY_CUSTOM_INJECT_MODULE_MAP.set(target, modules);
 ...
 * @returns
 */
function DirtyInjectProp(inject_module_getter, conf) {
...
exports.DirtyInjectProp = DirtyInjectProp;
...
function InjectProp(inject_module, conf = {}) {
    ...
    return (target, prop_name, des) => {
        if (autoResolvePlot !== 0 /* AUTO_RESOLVE_PLOT.NO_RESOLVE */ &&
            ...
            if (autoResolvePlot === 3 /* AUTO_RESOLVE_PLOT.AFTER_INIT */) {
                getInjectPropsMetadataMap(target, _inner_js_1.INJECTPROP_AFERTINIT_MATE_KEY).set(prop_name, {
                    injectModel: inject_module,
                });
            }
            else if (autoResolvePlot === 2 /* AUTO_RESOLVE_PLOT.ON_INIT */) {
                getInjectPropsMetadataMap(target, _inner_js_1.INJECTPROP_ONINIT_MATE_KEY).set(prop_name, {
                    injectModel: inject_module,
                });
            }
        }
        const getModule = (ins) => {
            const module_map = (0, _inner_js_1.getInstanceModules)(ins);
            if (!module_map) {
        ...
        const moduleGetter = (ins) => {
            ...
            if (module_map.has(module_name)) {
                ...
                return (0, Resolve_js_1.Resolve)(inject_module, module_map);
            }
