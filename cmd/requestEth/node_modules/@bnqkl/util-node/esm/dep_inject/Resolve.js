...
import { ModuleStroge } from "./ModuleStroge.js";
import { beforeCreateInstanceModuleStrogeStack, FACTORY_CUSTOM_INJECT_MODULE_MAP, INJECTION_TOKEN_SYMNOL, INJECTPROP_AFERTINIT_MATE_KEY, INJECTPROP_ONINIT_MATE_KEY, INSTANCE_MODULES_WM, SINGLETON_TOKEN_SYMNOL, } from "./_inner.js";
 ...
 */
export function Resolve(Factory, moduleMap = new ModuleStroge()) {
    /**这个构造器的标识名称 */
    const moduleName = Factory[INJECTION_TOKEN_SYMNOL];
    const isSingleton = Factory[SINGLETON_TOKEN_SYMNOL];
    ...
    if (!instance) {
        ...
        // 如果是单例模式，保存单例
        if (isSingleton) {
            Factory[SINGLETON_TOKEN_SYMNOL] = instance;
        }
        // 将模块缓存到map中
        ...
        const onInitMap = Reflect.getMetadata(INJECTPROP_ONINIT_MATE_KEY, Factory.prototype);
        if (onInitMap) {
        ...
        const afterInitMap = Reflect.getMetadata(INJECTPROP_AFERTINIT_MATE_KEY, Factory.prototype);
        if (afterInitMap) {
            queueMicrotask(() => {
